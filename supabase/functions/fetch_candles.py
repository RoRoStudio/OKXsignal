...
def fetch_timestamps(pair):
    """Fetch the latest and oldest timestamps for a pair."""
    latest_response = supabase.table("candles_1D").select("timestamp").eq("pair", pair).order("timestamp", desc=True).limit(1).execute()
    latest_timestamp = parser.isoparse(latest_response.data[0]['timestamp']) if latest_response.data else None

    oldest_response = supabase.table("candles_1D").select("timestamp").eq("pair", pair).order("timestamp").limit(1).execute()
...
def fetch_candles(pair, after_timestamp=None):
    """Fetch historical candles efficiently for the 1D timeframe."""
...
def main():
    ...
    print(f"🚀 Syncing {len(pairs)} trading pairs for 1D candles...")
