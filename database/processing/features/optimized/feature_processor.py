...
class OptimizedFeatureProcessor:
    ...
    def __init__(self, use_numba=True, use_gpu=False):
        ...
        if self.use_gpu:
            ...
            logging.info(f"GPU acceleration {'enabled' if self.gpu_initialized else 'failed to initialize'}")
        else:
            self.gpu_initialized = False
            if use_gpu and not self.gpu_available:
                logging.warning("GPU acceleration requested but not available")
                
        # Log initialization
        logging.info(f"Optimized feature processor initialized: "
